...
// 10 adet onaylanmış rezervasyon kaydı
const rows = Array(10).fill(0).map((_, index) => {
  const id = 2001 + index;
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        ...
        <Chip 
          label={`Toplam: ${rows.length} rezervasyon`} 
          color="primary" 
          variant="outlined" 
        />
      </Box>
      ...
      <TableContainer>
        <Table stickyHeader aria-label="rezervasyon listesi tablosu">
          <TableHead>
            <TableRow>
              ...
              <TableCell align="center" sx={{ backgroundColor: '#f5f5f5' }}>Durum</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {rows.map((row) => (
