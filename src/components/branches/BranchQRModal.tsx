...
const BranchQRModal: React.FC<BranchQRModalProps> = ({ open, onClose, branchId, branchName }) => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      downloadLink.download = `${branchName}-QR-Kodu.png`;
      downloadLink.click();
    } catch (error) {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      printWindow.document.write(`
        <html>
          <head>
            <title>Şube QR Kodu Yazdır</title>
            <style>
              body { 
                margin: 0; 
                display: flex;
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], `${branchName}-QR-Kodu.png`, { type: 'image/png' });

      await navigator.share({
        title: `${companyName} - ${branchName} Şube QR Kodu`,
        text: 'Bu QR Kodunu Okutarak Şube Sistemine Erişebilirsiniz',
        files: [file],
      });
    } catch (error: unknown) {
        ...
        {loading ? (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            >
              <Box sx={{ mb: 3 }}>
                <QRCodeCanvas
                  value={`${companyId}/${branchId}`} // Şirket ID ve Şube ID'yi birleştir
                  size={200}
              ...
              <Typography variant="body2" color="text.secondary">
                Bu QR Kodunu Okutarak<br />
                Şube Sistemine Erişebilirsiniz
              </Typography>
            </Paper>
