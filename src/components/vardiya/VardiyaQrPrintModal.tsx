...
const VardiyaQrPrintModal: React.FC<VardiyaQrPrintModalProps> = ({ open, onClose, companyId, branchId }) => {
  ...
  // A5 boyutları - aktif yönlendirmeye göre
  const getActiveDimensions = () => a5DimensionsByOrientation[orientation];
  ...
  const loadSavedLayout = () => {
    const storageKey = branchId ? `vardiyaQrLayout_branch_${branchId}` : `vardiyaQrLayout_company_${companyId}`;
  ...
  const handleAddQrCode = useCallback(() => {
    const qrId = branchId || companyId;
    
    if (!qrId) {
      console.error("Geçerli şirket veya şube ID'si yok");
      return;
    }
  ...
  }, [branchId, companyId, qrSize, orientation]);
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      const newItem = {
        id: `logo-${Date.now()}`,
        type: 'logo' as const,
        position: { x: dimensions.width / 2, y: dimensions.height / 4 },
        size: 100,
        content: file.name,
        imageUrl,
        selected: false
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (resizing && resizeItemId) {
      ...
      let newWidth = Math.max(100, Math.min(getActiveDimensions().width - 40, initialWidth + dx));
      
      // Sadece genişliği güncelle
      setPageItems(prev => prev.map(item => {
    ...
    const dimensions = getActiveDimensions();
    const boundedX = Math.max(minEdgeDistance, Math.min(dimensions.width - minEdgeDistance, newX));
    const boundedY = Math.max(minEdgeDistance, Math.min(dimensions.height - minEdgeDistance, newY));
      ...
      setTimeout(() => {
        ...
        .then(function (dataUrl) {
          ...
          printWindow.document.write(`
            ...
            <html>
              <head>
                <title>${branchId ? 'Şube' : 'Şirket'} QR Kodu</title>
  ...
  // Bilgisayara kaydetme fonksiyonu
    ...
    try {
      ...
      setTimeout(() => {
        ...
        .then(function (dataUrl) {
          ...
          link.download = `${branchId ? 'Şube' : 'Şirket'}_QR_Kodu.jpg`;
          link.href = dataUrl;
          link.click();
  ...
  // Modal kapanırken güncel düzeni kaydet
  ...
  return (
    ...
    >
      <DialogTitle sx={{ 
        ...
        <Typography variant="h6" fontWeight="bold">Vardiya QR Kodu Tasarla</Typography>
        ...
        <Box sx={{ display: 'flex', height: '100%', width: '100%' }}>
          ...
          >
            <Paper
              ...
              elevation={3}
              sx={{
                width: getActiveDimensions().width,
                height: getActiveDimensions().height,
...
export default VardiyaQrPrintModal; 