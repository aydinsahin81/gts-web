...
interface WeeklyPersonnelPerformanceChartProps {
  weeklyPersonnelPerformanceData: PerformanceData[];
  onInfoClick: () => void;
}

const WeeklyPersonnelPerformanceChart: React.FC<WeeklyPersonnelPerformanceChartProps> = ({ 
  weeklyPersonnelPerformanceData, 
  onInfoClick 
}) => {
    ...
    <ChartContainer>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="h6" fontWeight="bold">
          Personel Performansı - Haftalık Görevler
        </Typography>
      ...
      {weeklyPersonnelPerformanceData.length === 0 ? (
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 250 }}>
          <Typography variant="body2" color="textSecondary">Henüz haftalık personel performans verisi bulunmuyor</Typography>
        </Box>
      ) : (
        <ResponsiveContainer width="100%" height={250}>
          <BarChart
            data={weeklyPersonnelPerformanceData.slice(0, 5)}
...
export default WeeklyPersonnelPerformanceChart; 