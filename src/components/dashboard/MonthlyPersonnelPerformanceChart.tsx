...
interface MonthlyPersonnelPerformanceChartProps {
  monthlyPersonnelPerformanceData: PerformanceData[];
  onInfoClick: () => void;
}

const MonthlyPersonnelPerformanceChart: React.FC<MonthlyPersonnelPerformanceChartProps> = ({ 
  monthlyPersonnelPerformanceData, 
  onInfoClick 
}) => {
    ...
    <ChartContainer>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="h6" fontWeight="bold">
          Personel Performansı - Aylık Görevler
        </Typography>
      ...
      {monthlyPersonnelPerformanceData.length === 0 ? (
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 250 }}>
          <Typography variant="body2" color="textSecondary">Henüz aylık personel performans verisi bulunmuyor</Typography>
        </Box>
      ) : (
        <ResponsiveContainer width="100%" height={250}>
          <BarChart
            data={monthlyPersonnelPerformanceData.slice(0, 5)}
...
export default MonthlyPersonnelPerformanceChart; 